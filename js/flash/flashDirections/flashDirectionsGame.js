...
function jugarRapid(fase){
	...
	if(fase==1){
		...
		for(i=0;i<120;i++){
			...
			selectedItems[i] = {};
			selectedItems[i].txt = figura;
			selectedItems[i].color = color;
			selectedItems[i].mostrar = `<span style="font-size: 100px; color: #${color};"><b>${figura}</b></span>`;
	...
	if(cartasByFlash==1)
		$("#screen").html(`<br><br><center><b>${pasadas+1}</b></center><br><center>${selectedItems[pasadas].mostrar}</center>`);
	if(cartasByFlash==2){
		_p1=`${selectedItems[pasadas].mostrar}`;
		_p2="";
		if(pasadas+1<totalPasadas)
			_p2=`${selectedItems[pasadas+1].mostrar}`;

		$("#screen").html(`<br><br><center><b>${pasadas+1}</b></center><br><center>${_p1}<span>&nbsp;</span>${_p2}</center>`);
	}
	if(cartasByFlash==3){
		_p1=`${selectedItems[pasadas].mostrar}`;
		_p2="";
		...
		if(pasadas+2<totalPasadas)
			_p3=`${selectedItems[pasadas+2].mostrar}`;

		$("#screen").html(`<br><br><center><b>${pasadas+1}</b></center><br><center>${_p1}<span>&nbsp;</span>${_p2}<span>&nbsp;</span>${_p3}</center>`);
	}
